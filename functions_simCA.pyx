...
def integrand1(double y, double alpha, double beta, int mod):
	return y**(2.*beta-2.)*nu(y, mod)*get_M_NFW(y*alpha, mod)

def integral1(double ymin, double alpha, double beta, int mod):
	return quad(integrand1,ymin,+np.inf,args=(alpha, beta, mod),epsabs=1.e-2,epsrel=1.e-2)[0]

def integrand2(double z, double alpha, double beta, double gamma, int mod):
	result = (1.-beta/z**2)*z**(1.-2.*beta)/np.sqrt(z*z-1.)
	res = integral1(z*gamma, alpha, beta, mod)
	return result * res

def integral2(double gamma, double beta, double alpha, int mod):
	return quad(integrand2,1.,+np.inf,args=(alpha, beta, gamma, mod),epsabs=1.e-2,epsrel=1.e-2)[0]
