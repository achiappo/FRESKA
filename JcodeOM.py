...
#	PYTHON code to fit the J-factor of dSphs using Maximum Likelihood. This version assumes an OSIPKOV-MERRIT velocity distribution 
#	profile of the dSphs stellar component. Three possible stellar light profiles can be chosen from the Hernquist profile 
#	(select stelprof)  and two versions of the NFW DM profile: Cusped and Cored (select DMprof)
...
from functionsOM import integral2, Jfactor, nu, get_data
...
case = 'OM_%s_0%i_%s_%s'%(dwarf,theta*10,DMprof[DMmod],stelprof[stelmod])
...
class logLike:
    ...
    def compute(self,J,ra,r0):
        gamma = self.data
        I_array = np.array([integral2(gamma[i],ra/rh,rh/r0) for i in range(R.size)])
...
for J in J_array:
    ...
    settings = {'errordef':0.5,'print_level':0,'pedantic':False,'ra':rh,'r0':rt,'J':J,'fix_J':True,
    'error_ra':0.001,'error_r0':0.001,'limit_ra':(R.min(),R.max()),'limit_r0':(R.min(),rt*100)}
    ...
    if BF[0]['is_valid']:
    	...
    	min_ra_arr = np.append(min_ra_arr,BF[1][1]['value'])
