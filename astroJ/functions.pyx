...
def get_data( str gal ):
	...
	x,v,dv = np.loadtxt(
		'data/velocities/velocities_%s.dat'%gal , 
		dtype=float , 
		usecols = ( 0 , 1 , 2 ) , 
		unpack=True)
...
if params['DM_prof'] == 'Zhao' :
	...
	def rho( double x, tuple DM_params ):
		'''Zhao-Hernquist profile (x := r/r_s)'''
...
if params['stellar']=='Hernquist':
	...
	def I( double R , tuple ST_params):
		'''surface brightness profile from Hernquist profile'''

		return quad(integrand_I , R , +np.inf , args=(R , ST_params))[0]

	#----------------------------------------------------------------------
elif params['stellar']=='surf_bright':
	...
	if params['I_prof']=='Plummer':
		...
		def nu( r, tuple ST_params ):
			'''stellar density (from inverse 
	...
	#----------------------------------------------------------------------
		...
		def I( double gamma, tuple ST_params ):
			'''Sérsic profile for dSphs surface brightness'''
		...
		def nu( double y, tuple ST_params ):
			'''stellar density (from inverse 
				Abel transform of Sérsic profile)'''
...
if params['anisotropy']=='CA':
	...
	def Kernel( double u, double w, double beta ):
		'''Kernel function for constant 
		   anisotropy velocity distribution'''
...
if params['anisotropy']=='OM':
	...
	def Kernel(double u, double w, double beta):
		'''Kernel function for Osipkov-Merritt
		   velocity distribution'''
...
def integrand_s( double y, double gamma, 
	...
	return Kuw * nu(y , ST_params) * M(y * alpha , DM_params) / y
